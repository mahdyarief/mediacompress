...
export const VideoOutputDetails = ({
  ...
  const totalOriginalSize = videoFiles.reduce(
    (sum, file) => sum + file.fileSize,
    0
  );
  const totalOutputSize = videoFiles.reduce(
  ...
  const download = (file: FileActions) => {
    ...
    const downloadName = file.output
      ...
      : `${toSnakeCase(file.fileName)}.${videoSettings.videoType}`;
  ...
  const downloadAll = async () => {
    try {
      ...
      // Add all compressed files to ZIP with correct names and extensions
      for (const file of videoFiles) {
        if (file.outputBlob) {
          ...
          if (file.output) {
            ...
            // Fallback: construct filename based on video type
            const snakeCaseName = toSnakeCase(file.fileName);
            newFileName = `${snakeCaseName}.${videoSettings.videoType}`;
          }
      ...
      <div className="text-sm">
        <div className="flex justify-between items-center border-b mb-2 pb-2">
          <div className="flex items-center gap-1">
            <p className="">Output Files ({videoFiles.length})</p>
          ...
          {videoFiles.length > 0 && (
            ...
            >
              ...
              {videoFiles.length > 1 ? "Download All as ZIP" : "Download as ZIP"}
            </button>
          )}
        </div>
                  ...
                  <p className="text-xs font-medium truncate flex-1">
                    {file.output
                      ? file.output
                          .split("/")
                          .pop()
                          ?.toLowerCase() || `${toSnakeCase(file.fileName)}.${videoSettings.videoType}`
                      : `${toSnakeCase(file.fileName)}.${videoSettings.videoType}`}
                  </p>
                  <button
